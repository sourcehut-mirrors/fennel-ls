...
(λ main-loop [in out]
  ...
  (while true
    (let [msg (json-rpc.read in)]
      ...
      (while (next server.files-pending-lint)
        (let [msg (json-rpc.try-read in)]
          (if msg
              (dispatch.handle server send msg)
              (let [uri (next server.files-pending-lint)
                    file (files.get-by-uri server uri)]
                (lint.add-lint-diagnostics server file)
                (send (message.diagnostics file))
                (tset server.files-pending-lint uri nil))))))))

(λ handle-one-message [server msg]
  "handles a message, and returns all the responses in a table.
   used for tests"
  (let [out []]
    (dispatch.handle server (partial table.insert out) msg)
    (each [uri (pairs server.files-pending-lint)]
      (let [file (files.get-by-uri server uri)]
        (lint.add-lint-diagnostics server file)
        (table.insert out (message.diagnostics file))
        (tset server.files-pending-lint uri nil)))
    out))
